...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    if (!regexUser.test(username)) {
      abrirModalAvisoCadastro(
        "Insira um username válido! Apenas letras, números e underline (_), e no mínimo 4 caracteres!"
      );
      return;
    }
    ...
    fetch("../php/cadastrar.php", {
      ...
      credentials: "include",
      method: "POST",
      body: `nome=${encodeURIComponent(nome)}&username=${encodeURIComponent(
        username
      )}&senha=${encodeURIComponent(senha)}&email=${encodeURIComponent(email)}`,
    })
      .then((response) => {
      ...
      .then((data) => {
        ...
        if (data.sucesso === "true") {
          ...
          abrirModalAvisoCadastro(
            data.mensagem || "Erro desconhecido no cadastro"
          );
        }
      })
      .catch((error) => {
