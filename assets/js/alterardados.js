let opcoesAlterar = document.querySelectorAll(".alterar"); // Botões de alterar
let opcoesBtn = document.querySelectorAll(".opcoes-btn"); // Sair e excluir a conta
...
function validarEmail(emailAtual, novoEmail) {
  const regex = /^[\w\.-]+@[\w\.-]+\.\w+$/;
  return (
    regex.test(novoEmail) && novoEmail.length >= 4 && novoEmail !== emailAtual
  );
}
...
function validarLink(linkAtual, novoLink, tipo) {
  ...
  return (
    (regex.test(novoLink) && novoLink !== linkAtual) ||
    (novoLink == "" && novoLink !== linkAtual)
  );
}
  ...
  opcao.addEventListener("click", () => {
    ...
    if (texto === "Alterar links") {
      ...
      function atualizarEstadoBotaoLinks() {
        ...
        botao.style.boxShadow = algumValido
          ? "2px 2px 0px 1.5px #1453A3"
          : "none";
...
function abrirModalSair() {
  ...
  document.querySelector("#campoimagem").style.display = "none";
  document.querySelector("#campoaviso").style.display = "none";
  document.querySelector("#camposair").style.display = "flex";
  document.querySelector("#campoexcluir").style.display = "none";
  document.querySelector(".modal").style.width = "30%";
}
...
function abrirModalExcluir() {
  ...
  document.querySelector("#campoaviso").style.display = "none";
  document.querySelector("#camposair").style.display = "none";
  document.querySelector("#campoexcluir").style.display = "flex";
  document.querySelector(".modal").style.width = "30%";
}
...
function confirmarAlteracao() {
  ...
  if (titulo === "Alterar senha") {
    ...
    fetch("../php/alterarSenha.php", {
      ...
      credentials: "include",
      body: `senhaatual=${encodeURIComponent(
        atual
      )}&senhanova=${encodeURIComponent(nova)}`,
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  if (titulo === "Alterar links") {
    ...
    fetch("../php/alterarLinks.php", {
      ...
      credentials: "include",
      body: `github=${encodeURIComponent(github)}&linkedin=${encodeURIComponent(
        linkedin
...
function excluirConta() {
  fetch("../php/excluir.php", {
    ...
    .catch((err) => {
      abrirModalAviso("Erro ao excluir a conta!");
    });
}
